...
def hapi_to_df(dataA, round_to_sec=False, clean_time=False):
    ...
    # clean times
    dfA['Time'] = pd.to_datetime(dfA['Time'].str.decode("utf-8")) #hapitime2datetime(np.array(dfA['Time']),**ops)
    if round_to_sec:
...
def merge_hapi(dataA, metaA, dataC, metaC, round_to_sec=False, fill_nan=False, join_all=True):
    ...
    for ele in metaC['parameters']:
        if ele['name'] != 'Time' and (join_all or (ele not in metaA['parameters'])):
            ...
            if ele in metaA['parameters']:
                name = ele['name']
                new_name = f"{name}_{metaC['x_dataset']}" # does x_dataset always exist?
    ...
    newAC = np.array([tuple([nparray_unpack_to_list(e) for e in elm]) for elm in newAC],dtype=dt)
